...
func process_arms(delta: float) -> void:
	...
	ParticleLastPosition = lerp(ParticleLastPosition,position,0.7*delta)

	for arm: Node3D in arms:
		arm.look_at(last_position, Vector3.UP, false)

	for particlesystem : GPUParticles3D in arm_particles:
		particlesystem.amount_ratio = clamp(abs((ParticleLastPosition - position).length()*2),0.1,3)
		particlesystem.speed_scale = clamp(abs((ParticleLastPosition - position).length()*2),1,3)

var ParticleLastPosition = Vector3.ZERO
