...
var signalling_server_connection: int
...
func reset_connection() -> void:
	...
	for connection: WebRTCPeerConnection in connection_list.values():
		connection.close()
...
func parse_msg() -> bool:
	...
	if type == Message.ICE:
		...
		var sender_name: String = str_arr[2]
		var sender_id: int = id
		_ice_received(media, index, sender_name, sender_id)
		return true

	if type == Message.ANSWER:
		var str_arr: PackedStringArray = data.split("***", true, 2)
		var answer_type: String = str_arr[0]
		var sdp: String = str_arr[1]
		var sender_id: int = id
		#print("ANSWER from ", id)
		_answer_received(answer_type, sdp, sender_id)
		return true

	if type == Message.OFFER:
		var str_arr: PackedStringArray = data.split("***", true, 2)
		var offer_type: String = str_arr[0]
		var sdp: String = str_arr[1]
		var sender_id: int = id
		#print("OFFER from ", id)
		_offer_received(offer_type, sdp, sender_id)
		return true
...
func send_user_name(user_name: String) -> void:
	Helpers.log_print(str(" sending user name ", user_name))
	...
	send_msg(
		Message.USER_INFO,
		0,
		JSON.stringify({"name": user_name, "server_id_string": server_id_string})
	)
...
func send_offer(type: String, sdp: String, id: int) -> void:
	send_msg(Message.OFFER, 0, type + "***" + sdp + "***" + str(id))


func send_answer(type: String, sdp: String, id: int) -> void:
	send_msg(Message.ANSWER, 0, type + "***" + sdp + "***" + str(id))


func send_ice(media: String, index: int, _name: String, id: int) -> void:
...
func session_created(type: String, sdp: String, connection: WebRTCPeerConnection) -> void:
...
func ice_created(
	media: String, index: int, _name: String, connection: WebRTCPeerConnection
) -> void:
	#Helpers.log_print(str("ice_created ", media, " ", index, " ", _name))
	send_ice(media, index, _name, connection_list.find_key(connection))


func _ice_received(media: String, index: int, _name: String, sender_id: int) -> void:
...
func _offer_received(type: String, sdp: String, sender_id: int) -> void:
	#Helpers.log_print(str("_offer_received ", type, " ", sdp, " ", sender_id))
	if connection_list.has(sender_id):
		connection_list.get(sender_id).set_remote_description(type, sdp)


func _answer_received(type: String, sdp: String, sender_id: int) -> void:
