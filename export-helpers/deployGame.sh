...
if [[ "${REMOTE_HOST}" != "" ]]; then
  ...
  ssh.exe "${USER}@${REMOTE_HOST}" "chmod +x /mnt/2000/container-mounts/caddy/${GAME_NAME}/${GAME_NAME}.x86_64;chmod +x /mnt/2000/container-mounts/caddy/${GAME_NAME}/restart-server.sh;chmod +x /mnt/2000/container-mounts/caddy/${GAME_NAME}/run-server.sh;cd /home/${USER}/containers/caddy;docker compose up --detach --build ${GAME_NAME}"
fi
