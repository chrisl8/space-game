...
print_usage() {
  echo "Godot Game Server Run Script"
  echo "This is a script that I use to run my multiplayer web-based game server on the host."
  ...
  echo ""
  echo "Example Usage:"
  echo "run-server.sh --game-name game"
}
...
cd "${SCRIPT_DIR}" || exit
./"${GAME_NAME}.x86_64" --headless -- server
